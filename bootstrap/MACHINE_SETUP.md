...
- **rpiboot / usbboot** (2026-09-22): інсталятор з GitHub `raspberrypi/usbboot` (releases, Windows installer) → `C:\Program Files (x86)\Raspberry Pipiboot.exe` (`-d mass-storage-gadget64` / `-d msd`); для прошивки eMMC CM4. Примітка: на цьому ПК другий етап USB-boot CM4 обривається (те саме на RPi 4) — див. пам'ять/KWS-021.
- **Raspberry Pi Imager** (2026-09-22): `winget install --id RaspberryPiFoundation.RaspberryPiImager -e` — образи для RPi/CM4 з попередніми налаштуваннями (користувач, SSH-ключ, hostname).
...
- **WSL 2 + Debian** (рішення Gans 2026-09-21 — **ставити при переїзді на інший ПК**, на нинішній машині не встановлено): `wsl --install -d Debian` (потрібна віртуалізація в BIOS і перезавантаження), далі в Debian `sudo apt install nftables systemd`-утиліти. Навіщо: перевірка синтаксису nftables (`nft -c -f`), `systemd-analyze verify` для юнітів, bash-скрипти розгортання — щоб L1 міг перевіряти Linux-конфігурацію локально, а не лише Архітектор на цільовому пристрої.
